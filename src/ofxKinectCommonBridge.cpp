		...
		if(bMappingDepthToColor)
		{
			...
			if(SUCCEEDED(mapResult))
			{
				...
				if (colorFormat.dwWidth != depthFormat.dwWidth || colorFormat.dwHeight != depthFormat.dwHeight) {
					ofLogError("ofxKinectCommonBridge::update") << "Mapping depth frame to color frame is not support yet for different resolutions";
					return;
				}
			...
			for (int i = 0; i < (depthFormat.dwWidth*depthFormat.dwHeight); i++) {
				depthPixels[i] = depthPixelsNuiWrap[i].depth;
				depthPlayerPixels[i] = depthPixelsNuiWrap[i].playerIndex;

				if (bComputingDepthImage) {
					depthImage[i] = depthLookupTable[ofClamp(depthPixelsNuiWrap[i].depth, 0, depthLookupTable.size() - 1)];
				}
			}
		}
...
ofPixels & ofxKinectCommonBridge::getDepthImageRef(){       	///< grayscale values
	if (!bComputingDepthImage) {
		ofLogError("ofxKinectCommonBridge::getDepthImageRef") << "The computation of a depth image has not been set";
		return ofPixels();
	}
	return depthImage;
}
...
//------------------------------------
ofShortPixels & ofxKinectCommonBridge::getDepthPixelsRef(){
	return depthPixels;
}
...
void ofxKinectCommonBridge::drawDepth(float _x, float _y) {
	drawDepth(_x, _y, (float)colorFormat.dwWidth, (float)colorFormat.dwHeight);
}
...
void ofxKinectCommonBridge::drawDepthImage(float _x, float _y, float _w, float _h) {
	if (!bComputingDepthImage) {
		ofLogError("ofxKinectCommonBridge::drawDepthImage") << "The computation of a depth image has not been set";
		return;
	}
	if(bUseTexture) {
...
bool ofxKinectCommonBridge::initDepthStream( int width, int height, bool nearMode, bool mapDepthToColor )
{
	...
	bMappingDepthToColor = mapDepthToColor;
...
bool ofxKinectCommonBridge::createDepthPixels( int width, int height )
{
    ...
    if( hKinect != 0 )
	{
		depthPixelsNuiWrap = new NUI_DEPTH_IMAGE_PIXEL[(depthFormat.dwWidth*depthFormat.dwHeight)];
		if (bComputingNuiFullDepth) {
		...
		depthPlayerPixels.allocate(depthFormat.dwWidth, depthFormat.dwHeight, OF_IMAGE_GRAYSCALE);
		...
		if(bUseTexture)
		{
			if(bProgrammableRenderer)
			{
				...
				depthTex.allocate(depthPixels, true);
			...
			else
			{
				...
				depthTex.allocate(depthFormat.dwWidth, depthFormat.dwHeight, GL_LUMINANCE16);
			}
		}
...
{
	bMappingColorToDepth = mapColorToDepth;
	if(bMappingColorToDepth && mapper == NULL)
	{
		/*
...
void ofxKinectCommonBridge::stop() {
	if(bStarted){
		...
		if(depthPixelsNuiWrap){
			delete[] depthPixelsNuiWrap;
		}
